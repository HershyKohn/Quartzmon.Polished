...
namespace Quartzmon
{
    ...
    {
        public static void UseQuartzmon(this IAppBuilder app, QuartzmonOptions options, Action<Services> configure = null)
        {
        ...
        private static void UseFileServer(this IAppBuilder app, QuartzmonOptions options)
        {
            IFileSystem fs;
            if (string.IsNullOrEmpty(options.ContentRootDirectory))
                fs = new FixedEmbeddedResourceFileSystem(Assembly.GetExecutingAssembly(), nameof(Quartzmon) + ".Content");
            else
