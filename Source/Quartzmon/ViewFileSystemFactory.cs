...
namespace Quartzmon
{
    ...
    {
        public static ViewEngineFileSystem Create(QuartzmonOptions options)
        {
        ...
        private class EmbeddedFileSystem : ViewEngineFileSystem
        {
            ...
            string GetFullPath(string filePath)
            {
                return Path.Combine(nameof(Quartzmon) + ".Views", filePath.Replace("partials/", "Partials/")).Replace('/', '.').Replace('\\', '.');
            }
        }
